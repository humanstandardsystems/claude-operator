...
[ -f "$TEACH_FLAG" ] && echo "TEACHING MODE ACTIVE — Before every action, briefly explain in plain language: what you are about to do, why, and what the expected result is. Keep it jargon-free. Always prefix the explanation with the label [LEARNING YOU A THANG] on its own line so the user knows it's a teaching message, not a system message. Then proceed with the action."
